...
import static io.ballerina.observe.choreo.Constants.EXTENSION_PROPERTIES_FILE;
import static io.ballerina.observe.choreo.Constants.PACKAGE_NAME;
import static io.ballerina.observe.choreo.Constants.PACKAGE_ORG;
import static io.ballerina.observe.choreo.Constants.PACKAGE_VERSION_PROPERTY_KEY;

/**
 * This is the open tracing extension class for {@link TracerProviderFactory}.
 *
 * @since 2.0.0
 */
public class ChoreoTracerProviderFactory implements TracerProviderFactory {
    ...
    public BObject getProviderBObject() {
        ...
        return ValueCreator.createObjectValue(prometheusModule, "TracerProvider");
    }

    @Override
    public TracerProvider getProvider() {
        if (provider == null) {
            provider = new ChoreoTracerProvider();
        }
        return provider;
    }
}
